...
@file=glob("tmpdir/fd/res2000/accession_based/out100SiteW.WT_*");
...
foreach $win(sort keys %$dat1){
	...
	foreach $acc(sort keys %{$dat1->{$win}}){
		...
		$i1="../1000EC/tmpdir/geneticLoad_acc_noD/dAllele.$acc";
		$i2="../1000EC/tmpdir/geneticLoad_acc_noD/synAllele.$acc";
		$i3="../1000EC/tmpdir/geneticLoad_acc_noD/nsynAllele.$acc";
		$out="tmpdir/bb$acc";
		
		`bedmap --echo --skip-unmapped $i1 tmpdir/aa0 > $out`;
		open MM,"$out" or die "l";
		$d=@tem=<MM>;
		close MM;

		`bedmap --echo --skip-unmapped $i2 tmpdir/aa0 > $out`;
		open MM,"$out" or die "l";
		$syn=@tem=<MM>;
		close MM;

		`bedmap --echo --skip-unmapped $i3 tmpdir/aa0 > $out`;
		open MM,"$out" or die "l";
		$nsyn=@tem=<MM>;
		close MM;

		print NN "$win\t$acc\tI\t$d\t$syn\t$nsyn\n";
...
foreach $win(sort keys %$dat2){
	...
	foreach $acc(sort keys %{$dat2->{$win}}){
		...
		print NNN "$win\t$acc\tNI\t$d\t$syn\t$nsyn\n";
		$pm->finish;
	}
	$pm->wait_all_children;
