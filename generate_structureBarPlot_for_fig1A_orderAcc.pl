...
foreach $in(@file){
	...
	$inn=$in;
	$inn=~s/Plot$/HumanCheck/;
	$cid=getCluster($inn);


	print NN "plotQ(iw,grplab=poplab,sortind ='$cid',grplabsize = 1,grplabangle = 70,grplabjust = 1,grplabpos = 1,linetype = 2,linesize=0.1,grplabheight = 1.7,pointsize = 1.5, divsize=0.1,divalpha = 0.2 )\n\n";
...
sub getCluster{
	...
	foreach $ee(@all){
		chomp $ee;
		@tm=split/\t/,$ee;
		next if $tm[1] ne "wildEmmer";
		for($x=3;$x<@tm;$x++){
			
			#cut off to determine the cluster of wild emmer ancestor
			#important!!!
			next if $tm[$x] < 0.5;
			$y=$x-2;
			$rec{$y}++;
	...
	$n=@tm=keys %rec;
	print "$n cluster of wild emmer ancestry is detected in $ff\n";
